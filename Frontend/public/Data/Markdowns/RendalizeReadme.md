...
パイプラインというのは独自のマークダウンを解釈するためのフィルターです
それは独自のマークダウンの種類分のフィルターをかけ、順々に解釈していって最終的に本来のマークダウンと独自のマークダウンが混ざったファイル→pipeline1→pipeline2→pipeline3→pipeline4→jsonの配列みたいな感じにしてそれをMarkdownEngineで描画するというものです
